  ...
  return (
    <div className="border-b border-[color-mix(in_srgb,var(--ax-line)_78%,transparent)] bg-[color-mix(in_srgb,var(--ax-surface)_94%,var(--ax-canvas))] text-[var(--ax-text-soft)]">
      <div className="mx-auto flex h-7 w-full max-w-[1520px] items-center justify-between gap-4 px-6 sm:px-8 lg:px-12 xl:px-16 2xl:px-20">
        <a href={getEcosystemHref("science", currentApp, activeProjectId)} className="shrink-0 text-[9px] font-semibold uppercase tracking-[0.17em] text-[var(--ax-accent)]/85">
          {ECOSYSTEM_NAME}
        </a>
        ...
        <a href={getEcosystemHref("science", currentApp, activeProjectId)} className="hidden min-w-0 max-w-[280px] items-center gap-2 text-[9.5px] sm:flex">
          <span className="text-[var(--ax-text-faint)]">Project</span>
          <span className="truncate font-medium text-[var(--ax-text-soft)]">{activeProjectTitle || (activeProjectId ? "Active project" : "Local workspace")}</span>
        </a>
      </div>
    </div>
  );
