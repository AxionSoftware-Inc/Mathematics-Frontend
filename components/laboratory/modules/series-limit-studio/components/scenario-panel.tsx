...
import type { SeriesLimitSavedScenario } from "../types";
    ...
    return (
        <div className="rounded-3xl border border-border/50 bg-background p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between gap-3">
                <div>
                    <div className="text-[10px] font-black uppercase tracking-[0.18em] text-accent">Scenario Library</div>
                    <div className="mt-2 text-sm leading-7 text-muted-foreground">
                        Turli limit, series va power-series holatlarini snapshot qilib solishtiring.
                    </div>
                </div>
                <div className="rounded-full border border-border/60 bg-background/70 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">
                    {state.savedScenarios.length} saved
                </div>
            </div>

            <div className="flex gap-3">
                <input
                    value={state.scenarioLabel}
                    onChange={(event) => state.setScenarioLabel(event.target.value)}
                ...
                <button
                    type="button"
                    onClick={state.saveCurrentScenario}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                {state.savedScenarios.length ? state.savedScenarios.map((item) => (
                    <div key={item.id} className="rounded-2xl border border-border/60 bg-background/55 px-4 py-3">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0" onClick={() => state.loadSavedScenario(item)}>
                                ...
                                <div className="mt-2 text-xs text-muted-foreground">
                                    aux = {item.auxiliaryExpression || "-"} {item.result ? `| result: ${item.result}` : ""}
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={() => state.removeSavedScenario(item.id)}
